...
function renderAllSteps() {
    ...
    document.getElementById('step-language').innerHTML =
        `<h3 class='step-heading'>1. Language</h3>
    ...
    document.getElementById('step-train-size').innerHTML =
        `<h3 class='step-heading'>2. Training Corpus Size</h3>
    ...
    document.getElementById('step-algo').innerHTML =
        `<h3 class='step-heading'>3. Algorithm</h3>
    ...
    document.getElementById('step-feature').innerHTML =
        `<h3 class='step-heading'>4. Feature for Training</h3>
        <select name='feature' id='feature' disabled>
            <option value='null'>---Select Feature for Training---</option>
        </select>`;
    // Accuracy
    document.getElementById('step-accuracy-container').innerHTML =
        `<button id='check-accuracy' class='sim-button' disabled>Check Accuracy</button>`;
    document.getElementById('result-content').innerHTML = '';
}

// Setup step logic for the new layout
function setupStepLogic() {
    let lang = null, train = null, algo = null, feature = null, data = null;
    ...
    document.getElementById('train').addEventListener('change', function() {
        ...
        if (train !== 'null') {
            ...
            const algos = [...new Set(data.filter(row => row.train_token === train).map(row => row.algo))];
    ...
    document.getElementById('algo').addEventListener('change', function() {
        ...
        if (algo !== 'null') {
            ...
            const features = [...new Set(data.filter(row => row.train_token === train && row.algo === algo).map(row => row.feature))];
    ...
    document.getElementById('check-accuracy').addEventListener('click', function() {
        if (lang && train && algo && feature && data) {
            const row = data.find(row => row.train_token === train && row.algo === algo && row.feature === feature);
    ...
    eng: [
        {
            ...
            taggedHtml: 'The <span class="tag-sample">DET</span> quick <span class="tag-sample">ADJ</span> brown <span class="tag-sample">ADJ</span> fox <span class="tag-sample">NOUN</span> jumps <span class="tag-sample">VERB</span> over <span class="tag-sample">ADP</span> the <span class="tag-sample">DET</span> lazy <span class="tag-sample">ADJ</span> dog <span class="tag-sample">NOUN</span> . <span class="tag-sample">.</span>'
        },
        {
            text: 'She reads a book every day.',
            taggedHtml: 'She <span class="tag-sample">PRON</span> reads <span class="tag-sample">VERB</span> a <span class="tag-sample">DET</span> book <span class="tag-sample">NOUN</span> every <span class="tag-sample">DET</span> day <span class="tag-sample">NOUN</span> . <span class="tag-sample">.</span>'
        },
        {
            text: 'Cats sleep on soft mats.',
            taggedHtml: 'Cats <span class="tag-sample">NOUN</span> sleep <span class="tag-sample">VERB</span> on <span class="tag-sample">ADP</span> soft <span class="tag-sample">ADJ</span> mats <span class="tag-sample">NOUN</span> . <span class="tag-sample">.</span>'
        }
    ],
    hin: [
        {
            text: 'यह एक सुंदर किताब है।',
            taggedHtml: 'यह <span class="tag-sample">PRON</span> एक <span class="tag-sample">NUM</span> सुंदर <span class="tag-sample">ADJ</span> किताब <span class="tag-sample">NOUN</span> है <span class="tag-sample">VERB</span> । <span class="tag-sample">.</span>'
        },
        {
            text: 'राम स्कूल जाता है।',
            taggedHtml: 'राम <span class="tag-sample">NOUN</span> स्कूल <span class="tag-sample">NOUN</span> जाता <span class="tag-sample">VERB</span> है <span class="tag-sample">VERB</span> । <span class="tag-sample">.</span>'
        },
        {
            text: 'बिल्ली दूध पीती है।',
            taggedHtml: 'बिल्ली <span class="tag-sample">NOUN</span> दूध <span class="tag-sample">NOUN</span> पीती <span class="tag-sample">VERB</span> है <span class="tag-sample">VERB</span> । <span class="tag-sample">.</span>'
        }
    ]
};
...
function setupInteractiveDemo() {
    ...
    function populateDemoDropdown(lang) {
        selectEl.innerHTML = '<option value="placeholder">-- Choose an example --</option>';
...
setupStepLogic = function() {
    ...
    if (checkBtn) checkBtn.addEventListener('click', function() {
        ...
        if (resultContent && resultContent.textContent.trim().startsWith('Accuracy is:')) {
